...
def create_match_metrics(match_analysis: Dict[str, Any]):
    """Create a visualization of match metrics from job matching analysis"""
    if not match_analysis or "analysis" not in match_analysis:
    ...
    for category in categories:
        if category in match_analysis["analysis"]:
            scores[category] = match_analysis["analysis"][category]["score"]
        else:
            scores[category] = 0
    ...
    with col1:
        create_score_gauge(match_analysis.get("match_score", 0), "Overall Match Score")
    
    with col2:
        st.markdown("<h3 style='color:white;'>Key Match Metrics</h3>", unsafe_allow_html=True)
        for category, score in scores.items():
            create_progress_bar(category.capitalize(), score, "auto")
    
    # Display strengths and considerations
    st.markdown("<h3 style='color:white;'>Key Strengths</h3>", unsafe_allow_html=True)
    for strength in match_analysis.get("key_strengths", [])[:3]:
        st.markdown(f"- {strength}")

    st.markdown("<h3 style='color:white;'>Areas for Consideration</h3>", unsafe_allow_html=True)
    for area in match_analysis.get("areas_for_consideration", [])[:3]:
        st.markdown(f"- {area}")

def create_cultural_fit_metrics(decision_data: Dict[str, Any]):
    """Create cultural fit score metrics based on decision feedback"""
